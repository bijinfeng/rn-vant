...
type Props = {
   ...
   * ```
   */
  renderScene: (props: { route: Route; jumpTo: (key: string) => void }) => React.ReactNode | null;
  /**
...
const BottomNavigation = ({
  ...
  const keyboardWillShow = React.useRef<EmitterSubscription>();
  const keyboardWillHide = React.useRef<EmitterSubscription>();
  const keyboardDidShow = React.useRef<EmitterSubscription>();
  const keyboardDidHide = React.useRef<EmitterSubscription>();
  ...
  if (!loaded.includes(focusedKey)) {
    ...
    setLoaded(_loaded => [..._loaded, focusedKey]);
  }
  ...
  React.useEffect(() => {
    if (Platform.OS === 'ios') {
      keyboardWillShow.current = Keyboard.addListener('keyboardWillShow', handleKeyboardShow);
      keyboardWillHide.current = Keyboard.addListener('keyboardWillHide', handleKeyboardHide);
    } else {
      keyboardDidShow.current = Keyboard.addListener('keyboardDidShow', handleKeyboardShow);
      keyboardDidHide.current = Keyboard.addListener('keyboardDidHide', handleKeyboardHide);
    }

    return () => {
      if (Platform.OS === 'ios') {
        keyboardWillShow.current?.remove();
        keyboardWillHide.current?.remove();
      } else {
        keyboardDidShow.current?.remove();
        keyboardDidHide.current?.remove();
      }
    };
  ...
  const isDark =
    typeof approxBackgroundColor === 'string' ? !color(approxBackgroundColor).isLight() : true;
              ...
              return renderTouchable({
                ...
                children: (
                  <View pointerEvents="none">
                    <Animated.View style={[styles.iconContainer, { transform: [{ translateY }] }]}>
                      <Animated.View style={[styles.iconWrapper, { opacity: activeOpacity }]}>
                        {renderIcon ? (
                          ...
                          <Icon name={route.icon as IconNames} color={activeTintColor} size={24} />
                        )}
                      </Animated.View>
                    ...
                    {labeled ? (
                      <Animated.View
                        style={[styles.labelContainer, { transform: [{ scale: _scale }] }]}
                      >
